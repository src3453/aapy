...
class multi:
    ...
    def dither(self,img,level,algo=ALGO_FLOYD_STEINBERG,order="3x3")->np.ndarray[np.float64]:
        ...
        ods = []
        tw, th, _ = self.get_dither_table(odtype, 255, 0)
        ...
        if algo == ALGO_BAYER:
            ...
            for v0, v1, od in ods:
                mask = (src >= v0) & (src <= v1)
                dy, dx = np.meshgrid((np.arange(height) % th).astype(int), (np.arange(width) % tw).astype(int), indexing='xy')
                v_less_than_od = src < np.array(od)[dy,dx]
                dst = np.where(mask & v_less_than_od, v0, dst)
                dst = np.where(mask & ~v_less_than_od, v1, dst)
        return dst
